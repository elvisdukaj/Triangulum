...
void MovementSystem::update(EntityManager &entities,
                            ...
                            double dt)
{
   entities.each<Motion, Position>([this, dt](Entity entity, Motion& motion, Position& position)
   {
                                     ...
                                     const sf::Vector2f &velocity)
{
   if (velocity.x > 0.1f)
   {
      pAC->setAnimation(AnimationId(AnimationType::Movement, RightMovementAnimation));

   }
   else if (velocity.x < -0.1f)
   {
      pAC->setAnimation(AnimationId(AnimationType::Movement, LeftMovementAnimation));
   }
   else
   {
      pAC->setAnimation(AnimationId(AnimationType::Movement, IdleMovementAnimation));
   }
}
