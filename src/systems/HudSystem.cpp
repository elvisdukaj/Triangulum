...
void HudSystem::update(EntityManager& entities, EventManager&, double dt)
{
    entities.each<SpaceShip, Health, Gun>(
        [this, dt](Entity entity, SpaceShip& spaceShip, Health& health, Gun& gun)
    {
        m_heatBorderSprite.setPosition(50.0f, 485.0f);
        m_window.draw(m_heatBorderSprite);

        m_heatBarSprite.setPosition(50.0f, 485.0f);
        m_heatBarRect.width = static_cast<int>(gun.heat + 0.5f);
